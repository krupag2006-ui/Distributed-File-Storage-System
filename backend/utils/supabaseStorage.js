...
const parseChunkIndexFromName = (name) => {
  const match = String(name || '').match(/chunk[_-](\d+)/i);
...
const listChunksFromCloud = async (fileId, bucket = supabasePrimaryBucket) => {
  requireSupabaseConfig();

  const folderPath = `uploads/${fileId}`;
  const { data, error } = await supabase.storage
    .from(bucket)
    .list(folderPath, {
      limit: 1000,
  ...
  if (error) {
    throw new Error(`Supabase chunk list failed for ${bucket}/${folderPath}: ${error.message}`);
  }

  return (data || [])
    .filter((item) => item.name && !item.name.endsWith('/'))
    .map((item) => {
      const chunkIndex = parseChunkIndexFromName(item.name);
      const chunkPath = `${folderPath}/${item.name}`;
    ...
    .filter((item) => Number.isInteger(item.chunkIndex))
